...
echo '<a href="'.get_category_link($category[0]->term_id ).'">'.$category[0]->cat_name.'</a>'; } ?></div>
  ...
  <div class="nwrap"><? if(has_post_thumbnail()) {?>
  <aside class="tumb"><? the_post_thumbnail(); ?></aside>
  <section><?php the_excerpt();?></section><? } else { ?>
  <section><?php the_excerpt();?></section><? } ?></div>
  <footer>
  <img src="<?php bloginfo('siteurl'); ?>/wp-content/themes/bcstl/images/bcstl_readm.png" alt="En lire plus" width="317" height="40" usemap="#Map<?php the_ID(); ?>">
  ...
  <?php endwhile; else{echo "<p>Désolé aucun article ne convient</p>";}  endif; ?>
